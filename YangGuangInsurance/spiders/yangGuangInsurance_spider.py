...
import pandas as pd
import YangGuangInsurance.gl as gl
from bs4 import BeautifulSoup
from YangGuangInsurance.items import YangGuangInsuranceItem


class YangGuangInsuranceSpider(Spider):
    name = 'yangGuangInsurance'
    ...
    for each in range(0, len(df_name)):
        ...
        start_urls.append('https://osms.sinosig.com//#/caseDetails?policyCode={}&name={}'.format(gl.search_bdh,gl.search_name))


    def parse(self, response):
        if "苏州应时雨商务信息咨询有限公司" in response.text:
            item = YangGuangInsuranceItem()#获取字段，字段在items.py中定义了
            ...
            for data in zipped_data:
                ...
                # 姓名
                try:
                    item[person_name] = my_df[(my_df[1] == person_relationShip) & (my_df[3] == "移动电话")].values[0][1]
                except:
                    item[person_name] = ""
                # 手机号
                try:
                    item[person_mobile] = my_df[(my_df[1] == person_relationShip) & (my_df[3] == "移动电话")].values[0][0]
                except:
                    item[person_mobile] = ""
